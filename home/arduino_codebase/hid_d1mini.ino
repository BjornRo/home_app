...
#define PUBLISH_COMM "balcony/relay"
#define SUB_BALC "balcony/sensor/data"
#define SUB_BIKE "bikeroom/sensor/data"
...
#define SEND_RECV_BUFFER_SIZE 63
...
void msg_from_broker(char* topic, uint8_t* payload, unsigned int payload_len) {
    if (payload_len < 13 || payload_len >= SEND_RECV_BUFFER_SIZE) return;

    memcpy(payload_cpy, (char*)payload, payload_len);
    payload_cpy[payload_len] = '\0';
    if (string_equals((char*)SUB_BALC, topic) && read_simple_json(payload_cpy, payload_values) == 2) {
        dtostrf(payload_values[0], 5, 1, balcony_temp);
        dtostrf(payload_values[1], 5, 1, balcony_humid);
    } else if (string_equals((char*)SUB_BIKE, topic) && read_simple_json(payload_cpy, payload_values) == 1) {
